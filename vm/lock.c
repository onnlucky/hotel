...
INTERNAL tlHandle resumeWithLock(tlFrame* _frame, tlHandle res, tlHandle throw) {
    if (!res) return resumeWithUnlock(_frame, res, throw);
    ...
    if (!res) return null;
    return resumeWithUnlock(_frame, res, null);
}
